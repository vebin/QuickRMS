    ...
    public class AdminLayoutAttribute : ActionFilterAttribute 
    {
		...
		public AdminLayoutAttribute()
		{
			...
			if (user != null)
			{
                ...
                AppsService = container.GetExportedValueOrDefault<IAppsService>();
		...
		private List<SidebarMenuModel> InitSidebarMenu(User user)
		{
			...
			//取出所有选中的节点
            IQueryable<int> parentModuleIdList = null;
            IQueryable<int> childModuleIdList = null;
            if (RoleModulePermissionService != null && RoleModulePermissionService.RoleModulePermissions.Count()>0)
			    parentModuleIdList = RoleModulePermissionService.RoleModulePermissions.Where(t => RoleIds.Contains(t.RoleId) && t.PermissionId == null && t.IsDeleted == false).Select(t => t.ModuleId).Distinct();
            if (RoleModulePermissionService != null && RoleModulePermissionService.RoleModulePermissions.Count() > 0)
                childModuleIdList = RoleModulePermissionService.RoleModulePermissions.Where(t => RoleIds.Contains(t.RoleId) && t.PermissionId != null && t.IsDeleted == false).Select(t => t.ModuleId).Distinct();
